...
const UserDetails = ({ userId }: Props) => {
    ...
    const reviewSubmitHandler = async (): Promise<undefined | string> => {
        ...
        try {
            ...
            console.log("Submitting review:", { roomId, reviewValue: ratingValue, reviewText: ratingText });
            ...
            const errorMessage = error.response?.data || "Error submitting review. Please try again.";
    ...
    console.log("UserDetails - User ID:", userId);
    console.log("UserDetails - User Bookings:", userBookings);
    console.log("UserDetails - Bookings Error:", error);
    console.log("UserDetails - Bookings Loading:", isLoading);
            ...
            <div className="container mx-auto px-2 md:px-4 py-10">
                <div className="grid grid-cols-12 gap-10">
                    ...
                    <div className="md:hidden w-14 h-14 rounded-l-full overflow-hidden ">
                        ...
                        <ol
                                className={`${currentNav === "bookings" ? "text-green-600 dark:text-green-400" : "text-gray-700 dark:text-gray-300"
                        ...
                        <ol
                                className={`${currentNav === "amount" ? "text-green-600 dark:text-green-400" : "text-gray-700 dark:text-gray-300"
                                    } inline-flex mr-1 md:mr-5 items-center space-x-1 md:space-x-3 cursor-pointer hover:text-green-600 dark:hover:text-green-400 transition-colors`}
                    ...
                    {currentNav === "amount" ? (
                        ...
                        {process.env.NODE_ENV === 'development' && (
                            <div className="mt-8 p-4 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg">
                                <h3 className="text-lg font-semibold text-yellow-800 dark:text-yellow-200 mb-4">Debug Info</h3>
                                <div className="space-y-2 text-sm">
                                    <p><strong>User ID:</strong> {userId}</p>
                                    <p><strong>Session User:</strong> {user?.name} ({user?.email})</p>
                                    <p><strong>User Data:</strong> {userData ? 'Loaded' : 'Not loaded'}</p>
                                    <p><strong>User Data Name:</strong> {userData?.name || 'N/A'}</p>
                                    <p><strong>User Data About:</strong> {userData?.about || 'N/A'}</p>
                                    <p><strong>User Data Email:</strong> {userData?.email || 'N/A'}</p>
                                    <p><strong>User Data Created:</strong> {userData?._createdAt || 'N/A'}</p>
                                    <p><strong>Bookings:</strong> {userBookings ? `${userBookings.length} bookings` : 'No bookings'}</p>
                                    <p><strong>Payment Status:</strong> {searchParams.get('payment_status') || 'None'}</p>
                                </div>
                                <div className="flex gap-2 mt-4">
                                    <button
                                        onClick={async () => {
                                            try {
                                                ...
                                                if (!roomData.success) {
                                                    toast.error('Failed to get test room: ' + roomData.error);
                                                    return;
                                                }
                                    ...
                                    <button
                                        onClick={async () => {
                                            try {
                                                const response = await fetch('/api/test-token-permissions');
                                                const data = await response.json();
                                    ...
                                    <button
                                        onClick={async () => {
                                            try {
                                                ...
                                                if (data.success) {
                                                    toast.success(`Found ${data.totalUserBookings} user bookings, ${data.totalAllBookings} total bookings`);
                                                    alert(JSON.stringify(data, null, 2));
                                                } else {
                                    ...
                                    <button
                                        onClick={async () => {
                                            try {
                                                const response = await fetch('/api/check-latest-bookings');
                                                const data = await response.json();
