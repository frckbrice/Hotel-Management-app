...
const RatingModal = ({
  ...
  return (
    <div
      className={`fixed inset-0 z-[61] flex items-center justify-center transition-opacity duration-300 ${isOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"
        } bg-black/30 backdrop-blur-sm`}
    >
      <div className="bg-white w-full max-w-md mx-4 rounded-xl shadow-2xl p-6 space-y-5 animate-fadeIn">
        <h2 className="text-2xl font-semibold text-gray-800 text-center">Rate Your Experience</h2>
          ...
          <div className="flex gap-2 justify-center">
            {starValues.map((value) => (
              <button
                ...
                type="button"
                onClick={() => setRatingValue(value)}
                className={`text-2xl transition-colors ${ratingValue >= value ? "text-yellow-500" : "text-gray-300"
                  } hover:scale-110`}
              >
                <BsStarFill />
              </button>
