  ...
  switch (event.type) {
    case checkout_session_completed:
      ...
      try {
        ...
        console.log("Webhook - Room ID from metadata:", session.metadata?.hotelRoom);
        console.log("Webhook - User ID from metadata:", session.metadata?.user);
        console.log("Webhook - Room ID from metadata:", session.metadata?.hotelRoom);

        const bookingResult = await createBooking(bookingData);
        console.log("Webhook - Booking created successfully:", bookingResult);
        console.log("Webhook - Booking ID:", bookingResult?.result?._id || bookingResult?._id);

        // *   Update hotel Room
        console.log("Webhook - Updating hotel room:", session.metadata?.hotelRoom);
        ...
        return new NextResponse(`Booking creation failed: ${error}`, { status: 500 });
      }
    default:
